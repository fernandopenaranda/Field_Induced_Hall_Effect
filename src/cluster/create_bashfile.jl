...
This .sh file will call to `julia_script` with the preset directives.
"""
function create_bashfile_FA(divisions)
    julia_script = dirname(pathof(Field_Induced_Hall_Effect)) * "/cluster/cluster_evaluate_subcube_FA.jl"  # script each array job runs
    subcubes_file = pwd() * "/subcubes.jls"        # File that stores the domain integration partitions
    output_script = pwd() * "/submit_array_FA.sh"     # SLURM submission script
