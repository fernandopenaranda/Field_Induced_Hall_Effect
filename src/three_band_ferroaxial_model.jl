...
tp, tp_z is a first order. New parameters, t_ca, U"""
function threeband_ferroaxial_ham3d(k, μ = 0, Δ = 1, t = 1, tp = 0.5, tp_z = 0.5, tc = 0.5, U = 0, t_ca = 1, t_cb = 1)
    a = 1.0
...
ca(k, deltas) = f(k, deltas) 
cb(k, deltas) = f(k, -deltas) 
...
function threeband_d_ferroaxial_ham3d(t,tp,Δ,tpz,tc, U, t_ca, t_cb, q, dir) 
    a = 1.0
    a1, a2, a3, δ1, δ2, δ3 = lattice_vectors(a)
    ds = [δ1, δ2, δ3]
    dfk = df(q, ds, dir)
    ...
    dgk = dg(q, ds, dir)
    dnnk = dnn(q, ds, dir)
    return [tc*dnnk t*dfk+tp*dgk+tpz*dmk conj(t_ca*dcak); conj(t*dfk+tp*dgk+tpz*dmk) tc*dnnk conj(t_cb*dcbk); t_ca*dcak t_cb*dcbk 0]
end
...
dca(k, deltas, a) = df(k, deltas, a) 
dcb(k, deltas, a) = df(k, -deltas, a) 
...
function threeband_d2_ferroaxial_ham3d(t, tp, tpz,tc, U, t_ca, t_cb, q, dir1, dir2) # model with the AA hoppings in z
    a = 1.0
    a1, a2, a3, δ1, δ2, δ3 = lattice_vectors(a)
    ds = [δ1, δ2, δ3]
    d2fk = d2f(q, ds, dir1, dir2)
    d2gk = d2g(q, ds, dir1, dir2)
    d2cak = d2ca(q, ds, dir1, dir2)
    d2cbk = d2cb(q, ds, dir1, dir2)
    d2mk = d2m(q, ds,a3, dir1, dir2)
    off = t*(d2fk) + tp*d2gk + tpz * d2mk
    d2nnk= d2nn(q, ds, dir1, dir2)
    return [tc*d2nnk off conj(t_ca*d2cak); conj(off)  tc*d2nnk conj(t_cb*d2cbk); t_ca*d2cak t_cb*d2cbk 0]
end
...
d2ca(k, deltas, dir1, dir2) = d2f(k, deltas, dir1, dir2)
d2cb(k, deltas, dir1, dir2) = d2f(k, -deltas, dir1, dir2)
